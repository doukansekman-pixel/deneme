# Design brief — Hype Bar

## Update — forked from Vype Lounge (owner's explicit direction)

This is a second instance of the exact same site (same components, same
dark palette `#211a16`/`#c9a876`, same section structure) rebranded for a
different real venue: **Hype Bar Stadtallendorf**, Niederkleiner Str. 44,
35260 Stadtallendorf (real site: hype-bar.de). The owner explicitly asked
for the structure/palette to stay identical to Vype Lounge — only content
and images differ — overriding the earlier "second site must differ on
the anti-convergence axes" note below (that note predates this direction).

- **Real facts used** (scraped from hype-bar.de's static pages): address,
  opening hours (So-Do 16:00-02:00, Fr-Sa 16:00-03:00), Instagram
  (@hype_bar_lounge), the two real promotions (Ladies Night: Shisha +
  Cocktail from 18:00 Fridays, 13 €; Hype Special: Shisha + Softdrink
  16:00-19:00 daily, 10 €), and the full Datenschutz legal text.
- **Blocked**: the real menu (full item list) and all real photos live
  behind hype-bar.de's Wix-rendered JS widget, which this environment
  can't execute (WebFetch doesn't run JS; headless Chromium in this
  sandbox has no outbound network access at all). The starter menu ships
  anchored on the two confirmed real promotions plus a few modest generic
  categories (Shisha/Cocktails/Snacks) for the owner to complete from
  `/admin`, same as Vype Lounge's own initial launch.
- **Images**: since the real site's photos were unreachable, free-license
  stock photography (Unsplash) was used instead — a shisha lounge, a
  cocktail bar, and a leather-booth interior shot chosen to match the dark
  warm palette already locked for this design system. The logo is a
  plain generated wordmark ("HYPE", same gold-taupe accent color) since no
  real logo file was reachable either.
- **Built directly for the VPS/Docker deployment** (not through Higgsfield
  hosting) — same `bindings.server.ts` SQLite/local-disk adapter pattern
  as Vype Lounge, its own SQLite file and uploads volume, own admin
  session cookie name (`hype_admin_session`) so the two apps never
  collide if ever run on the same host.
