-- A few homepage text bits were still hardcoded in index.tsx (not
-- admin-editable): the "Karte" teaser heading/body/CTA, the "Besuch uns"
-- heading, and the second feature block's CTA label. Additive-only, and
-- the defaults match the copy that was previously hardcoded, so existing
-- pages render identically until the owner edits them from /admin.
